...
def MART_loss(adv_logits, natural_logits, target, beta):
    ...
    tmp1 = torch.argsort(adv_probs, dim=1)[:, -2:]
    new_y = torch.where(tmp1[:, -1] == target, tmp1[:, -2], tmp1[:, -1])
    loss_adv = F.cross_entropy(adv_logits, target) + \
...
class LocalUpdate(object):
    ...
    def train_test(self, dataset, idxs):
        """Split indexes for train and test sets."""
        idxs_train = idxs[:int(1.0 * len(idxs))]
        idxs_test = idxs[int(0.9 * len(idxs)):]
        trainloader = DataLoader(DatasetSplit(dataset, idxs_train),
                                 batch_size=self.args.local_bs, shuffle=True)
    ...
    def update_weights(self, model, global_round):
        """Standard local training (no local print)."""
        model.train()
        epoch_loss = []
    ...
    def update_weights_at(self, model, global_round):
        """Adversarial training local update (no local print)."""
        epoch_loss = []
        index = 0.0
        index_pgd = 0
        ...
        for iter in range(self.args.local_ep):
            batch_loss = []
            index, index_pgd = 0.0, 0
            for batch_idx, (images, labels) in enumerate(self.trainloader):
                images, labels = images.to(self.device), labels.to(self.device)

                # 生成对抗样本
                if self.method == 'AT':
                    x_adv, ka = attack.PGD(model, images, labels, eps, sts, self.args.num_steps,
                                           loss_fn="cent", category="Madry", rand_init=True)
                elif self.method == 'TRADES':
                    x_adv, ka = attack.PGD(model, images, labels, eps, sts, self.args.num_steps,
                                           loss_fn="kl", category="trades", rand_init=True)
                elif self.method == 'MART':
                    x_adv, ka = attack.PGD(model, images, labels, eps, sts, self.args.num_steps,
                                           loss_fn="cent", category="trades", rand_init=True)
                else:
                    x_adv, ka = images, 0
                ...
                if self.alg == 'FedProx' and global_round > 0:
                    ...
                    for w, w_t in zip(model.parameters(), self.anchor.parameters()):
                        proximal_term += (w - w_t).norm(2)
                    loss += (self.anchor_mu / 2) * proximal_term
    ...
    def update_weights_scaffold(self, model, c_global_model, c_local_model, global_round):
        """SCAFFOLD update (no local print)."""
        model.train()
        epoch_loss = []
        index, index_pgd = 0.0, 0
        ...
        for iter in range(self.args.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.trainloader):
                ...
                if self.alg == 'FedProx' and global_round > 0:
                    proximal_term = sum((w - w_t).norm(2) for w, w_t in zip(model.parameters(), self.anchor.parameters()))
                    loss += (self.anchor_mu / 2) * proximal_term
                ...
                net_para = copy.deepcopy(model.state_dict())
                for key in net_para:
                    net_para[key] -= local_lr * (c_global_para[key] - c_local_para[key])
                model.load_state_dict(net_para)
                cnt += 1
        ...
        return model.state_dict(), sum(epoch_loss) / len(epoch_loss), \
               index / len(self.trainloader), correct / total, c_local_model, c_delta_para, index_pgd / len(self.trainloader)

    def inference(self, model):
        """Returns inference accuracy and loss."""
        model.eval()
        loss, total, correct = 0.0, 0.0, 0.0
