...
if args.method == "dat":
    ...
    if args.dataset != "svhn":
      ...
      loss, pgd20_acc = attack.eval_robust(model, test_loader, perturb_steps=10, epsilon=8/255, step_size=2/255,loss_fn="cent", category="Madry", random=True)
      ...
      print('FGSM without Random Start Test Accuracy: {:.2f}%'.format(100. * fgsm_wori_acc))
      loss, pgd20_acc = attack.eval_robust(model, test_loader, perturb_steps=10, epsilon=4/255, step_size=1/255,loss_fn="cent", category="Madry", random=True)
      print('PGD20 Test Accuracy: {:.2f}%'.format(100. * pgd20_acc))
