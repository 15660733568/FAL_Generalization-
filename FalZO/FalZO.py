...
from utils import get_dataset, average_weights, exp_details, zo_aggregate_weights
...
def run_experiment(args, train_dataset, user_groups, test_dataset, device, val_loader, logger=None):
    ...
    for epoch in tqdm(range(args.epochs)):
        for epsilon in args.epsilon_list:
            ...
            # --------------- 关键修改：零阶优化权重聚合 ---------------
            global_weights = zo_aggregate_weights(
                local_weights=local_weights,
                val_loader=val_loader,
                model_class=type(global_models[epsilon]),
                device=device,
                steps=15, mu=0.05, num_dir=6, zo_lr=0.1,
                args=args
            )
...
if __name__ == '__main__':
    ...
    np.random.seed(42)
    val_indices = np.random.choice(len(train_dataset), size=800, replace=False)
    ...
    for run in range(num_runs):
        ...
        run_results, run_models = run_experiment(
            args, train_dataset, user_groups, test_dataset, device, val_loader, logger
        )
